...
void procesa_argumentos(int argc, char *argv[])
{
    ...
    if (tam_cola < 1)
    {
        fprintf(stderr, "Error: El tamaño de la cola debe ser mayor o igual a 1\n");
        exit(6);
    }
    ...
    {
        fprintf(stderr, "Error: El número de hilos trabajadores no es un número.\n");
        exit(7);
    }
    num_hilos_work = atoi(argv[3]);
    if (num_hilos_work < 1)
    {
        fprintf(stderr, "Error: El número de hilos trabajadores debe ser mayor o igual a 1\n");
        exit(8);
    }
...
void *Worker(int *id)
{
    ...
    while (1)
    {
        ...
        else // es  una peticion por UDP
        {
			...
			// A RELLENAR
            if ((strcmp(cmd, "GETCHUNK") == 0) && (aux != NULL))
            {
                sendStringUDP(pet->s, aux, tam, &pet->d_cliente);
                free(aux);
            }
            else
                sendStringUDP(pet->s, msg, strlen(msg), &pet->d_cliente);
        }
...
void RecepcionPeticiones(int sockTCP, int sockUDP)
{
    ...
    while (1) // Bucle infinito de atencion de mensajes
    {
        ...
        FD_SET(sockTCP, &fds);
        FD_SET(sockUDP, &fds);
        if (select(maximo + 1, &fds, NULL, NULL, NULL) < 0)
        {
        ...
        if (FD_ISSET(sockTCP, &fds))
        {
            ...
            else
            {
                ...
                strncpy(p->msg, bufferTCP, TAMMSG - 1);
                p->msg[TAMMSG - 1] = '\0';
                p->s = s;
                bzero(&p->d_cliente, sizeof(struct sockaddr_in));
                p->is_stream = CIERTO;
        ...
        if (FD_ISSET(sockUDP, &fds))
        {
            ...
            else
            {
                ...
                strncpy(p->msg, bufferUDP, TAMMSG - 1);
                p->msg[TAMMSG - 1] = '\0';
                p->s = sockUDP;
...
int main(int argc, char *argv[])
{
    ...
    {
        int valor_semdesc;
        valor_semdesc = (int)(num_max_desc / 2) - 5;
        if (valor_semdesc < 1)
            valor_semdesc = 1;
        if (sem_init(&semdesc, 0, valor_semdesc) != 0)
        {
            perror("Error al inicializar semaforo de descriptores");
            exit(17);
        }
    }
    ...
    for (i = 0; i < num_hilos_work; i++)
    {
        ...
        if (id == NULL)
        {
            fprintf(stderr, "ERROR: No se pudo reservar memoria para id de hilo\n");
            exit(18);
        }
        *id = i;
        if (pthread_create(&hilos_work[i], NULL, (void *(*)(void *))Worker, id) != 0)
        {
            fprintf(stderr, "ERROR: No se pudo crear el hilo worker %d\n", i);
            free(id);
            exit(19);
        }
    }
